...
本文档提供了四个完整的使用示例，展示不同类型文章的创作流程和最佳实践。
...
```
请根据rss_aggregate获取到的股评文章，对今日A股市场表现进行总结和改写
```
...
#### 第1步：理解需求
- **主题类型**：股评总结和改写
- **核心角度**：基于已有RSS聚合数据进行分析和改写
- **目标读者**：普通投资者、股市关注者

#### 第2步：获取本地数据（跳过搜索）
- **直接使用**：`rss_for_tech_writer.md`
- **数据来源**：rss_aggregate技能从淘股吧、雪球等平台爬取的股评文章
- **数据内容**：市场概况、个股分析、投资建议等



#### 第4步：股评内容改写创作（2000-3000字）
...
**具体执行**：
1. **市场概况**：从RSS数据中提取主要指数表现
   - 沪指、深成指、创业板指涨跌幅
   - 成交量、北向资金流向等关键数据

2. **核心观点**：整合RSS数据中的分析师观点
   - 主流机构的投资建议
   - 市场情绪分析

3. **行业分析**：汇总RSS数据中的板块分析
   - 领涨领跌板块
   - 行业基本面分析

4. **个股点评**：提炼RSS数据中的个股分析
   - 重点关注股票的技术指标
   - 基本面变化和投资逻辑

5. **风险提示**：根据RSS数据中的风险提示进行总结
   - 市场风险因素
   - 投资注意事项

6. **后市展望**：整合RSS数据中的后市预测
   - 技术面分析
   - 操作建议

**语言风格特点**（参考stock-writing-style.md）：
- 使用类比："像是在雾中航行，需要谨慎前行"
- 第二人称："如果你关注科技股"、"你可能会注意到"
- 专业术语通俗解释：将"MACD金叉"解释为"技术指标发出买入信号"
- 适度使用金融术语，但需解释清楚

#### 第5步：生成封面图（强制）

**分析主题**：
- 关键词：A股、市场分析、投资策略、今日总结
- 核心价值："把握今日A股脉搏，洞察投资机会"
- 目标情绪：专业、稳健、有洞察力

**选择配色**：金融类 → 红绿渐变（代表涨跌）+ 金色点缀（代表财富）

**构建提示词**（基于金融类模板）：
```
A stunning, eye-catching cover image for A-share market analysis article.

Design: vibrant gradient background from deep red (#dc2626, representing market decline) to bright green (#22c55e, representing market gains), with golden (#f59e0b) accent elements and subtle light effects creating depth.

Central visual elements (positioned behind text):
- 3D floating stock charts and financial graphs in glass morphism style, semi-transparent with frosted glass effect
- Each element contains a glowing icon: upward trending arrow, stock price graph, currency symbol, investment portfolio
- Elements connected by luminous golden (#f59e0b) energy lines creating a financial network visualization
- Soft particle system with small glowing dots scattered across the scene
- Light rays emanating from elements with lens flare effects

Text layout (CRITICAL - all text centered both horizontally and vertically):
- CENTER of image: Large bold title 'A股市场分析' in white, modern sans-serif font with subtle glow
- Directly below title in CENTER: Chinese subtitle '把握今日投资脉搏' in elegant font, golden (#f59e0b) color, slightly smaller, 90% opacity
IMPORTANT: All text must be centered in the MIDDLE of the image, not at top or bottom edges.

Visual effects:
- Bokeh depth of field effect in background
- Subtle vignette darkening at edges
- Glowing highlights on chart edges
- Professional color grading with high contrast
- Clean composition with breathing space around text

Style: ultra-modern, finance-focused, professional, magazine-quality, trustworthy
Visual mood: analytical, professional, insightful, stable, profitable
Color palette: red-green gradient (#dc2626 to #22c55e) with golden (#f59e0b) and white (#ffffff) accents
...
- Text positioned in CENTER (middle) of image both horizontally and vertically
- High quality, 16:9 aspect ratio
- Professional design suitable for financial article cover
```
...
**验证质量**：
✅ 中文清晰：主标题"A股市场分析"和副标题清晰可读
✅ 颜色鲜明：红绿渐变体现金融市场特色
✅ 视觉元素：股票图表、趋势箭头、投资图标都有
✅ 符合主题：一看就知道是股市分析相关

#### 第6步：内容配图（按需 - 生成1张市场趋势图）

**判断**：文章中有多个市场的数据对比和趋势分析 → 生成趋势图
...
**提示词**（基于金融类图表）：
```
A clean financial trend chart for stock market indices performance.
Design: line chart with 3 trend lines showing daily performance.
Style: professional financial data visualization with smooth curves.
Color scheme: red (#dc2626) for Shanghai index, blue (#3b82f6) for Shenzhen index, green (#22c55e) for ChiNext index.
Elements:
- X-axis: time intervals (morning, midday, afternoon)
- Y-axis: percentage change in Chinese (涨跌幅 %)
- 3 trend lines: showing different indices movements
- Current values: +1.2%, +1.8%, +2.3% marked on lines
- Legend: 沪指, 深成指, 创业板指 in Chinese
Visual style: modern, clean, professional financial presentation quality.
Background: white with subtle gradient.
All text in simplified Chinese, accurate and clear.
IMPORTANT: Use exact values +1.2%, +1.8%, +2.3%.
16:9 aspect ratio, high contrast for readability.
```
...
python scripts/generate_image.py \
  --prompt "[上述提示词]" \
  --api gemini \
  --output market-trend.png
```

#### 第7步：输出文章

**输出文件**：
- `A股市场今日总结.md`（文章主文件，约2500字）
- `cover.png`（封面图，金融主题）
- `market-trend.png`（市场趋势图）

**文章特色**：
- 标题专业：包含"市场分析"和"投资脉搏"等关键词
- 结构清晰：6个部分，每部分主题明确
- 通俗易懂：用生活化比喻解释金融概念
- 数据准确：使用RSS聚合的真实市场数据
- 风险提示：包含适当的投资风险提醒
- 链接纯文本格式：`数据来源：https://xueqiu.com/`（不是markdown超链接）
- 无额外章节：没有"参考资料"、"免责声明"等
